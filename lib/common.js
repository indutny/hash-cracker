...
function amplifyJSON(list, prefix, amp) {
  const res = [];
  for (let i = 0; i < amp; i++)
    res.push(toJSON(list.concat(prefix)));
  return `[${res.join(',')}]`;
}
exports.amplifyJSON = amplifyJSON;

function getProbe(spoof, list, prefix, amp) {
  return '[' + spoof.map((s) => {
    return toJSON([ prefix ].concat(s));
  }).join(',') + ',' + amplifyJSON(list, prefix, amp) + ']';
}
