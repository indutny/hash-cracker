...
async.forEachLimit(iter, PARALLEL, (_, callback) => {
  ...
  measure(probes, (err, res) => {
    ...
    totalAvg += res.total.avg;
    totalDev += res.total.stddev;
    totalCount++;
    ...
    console.log('Running avg=%d dev=%d', totalAvg / totalCount, totalDev / totalCount);
    perPos.forEach((p, i) => {
      console.log('  pos=%d count=%d avg=%d, dev=%d', i, p.count, (p.avg / p.count) - (totalAvg / totalCount), p.stddev / p.count);
    });
