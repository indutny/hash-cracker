...
Client.prototype.runProbe = function runProbe(keys, otherKey, callback) {
  let probe = common.getProbe(this.spoof, keys, otherKey, AMPLIFICATION);
  probe = zlib.deflateSync(probe).toString('base64');

  this.rolling += probe.length;
  console.error(this.rolling, probe.length * EXTRACT_COUNT);
