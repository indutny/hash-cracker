...
async.forEachLimit(iter, PARALLEL, (_, callback) => {
  const probes = [];
  offset = (Math.random() * 0xffffffff) | 0;
  for (let i = 17; i < 17 + PROBES_COUNT; i++)
    probes.push(key(i + offset));
  ...
  measure(probes, (err, res) => {
    ...
    res.results.map((item) => {
      ...
      p.count++;
      p.avg += item.item.avg - res.total.avg;
      p.stddev += item.item.stddev;
    });

    perPos.forEach((p, i) => {
      console.log('%d, %d, %d, %d', i, p.count, p.avg / p.count, p.stddev / p.count);
    });
    return;
